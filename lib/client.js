...
function useConfigWriter(scope) {
  return (0, import_react.useMemo)(() => {
    ...
    return {
      ...
      clear: (field) => {
        if (!scope.getSnapshot().writable) return Promise.resolve(false);
        return scope.unset(field).then(() => fieldValue(field) === void 0).catch(() => false);
      },
...
function WeatherSettingsSection(props) {
  ...
  const commit = (0, import_react4.useCallback)((fields, clears = []) => {
    ...
    const pending = [];
    for (const [field, value] of fields) pending.push(scope.set(field, value));
    for (const field of clears) pending.push(scope.unset(field));
    void Promise.all(pending).then(() => {
      const current = scope.getSnapshot().value;
      const accepted = current !== void 0 && fields.every(([field, value]) => JSON.stringify(current[field]) === JSON.stringify(value)) && clears.every((field) => current[field] === void 0);
        ...
        suggestions.length > 0 && /* @__PURE__ */ (0, import_jsx_runtime5.jsx)(
          "div",
          {
            ...
            children: suggestions.map((place) => {
              ...
              return /* @__PURE__ */ (0, import_jsx_runtime5.jsxs)("div", { style: { display: "flex", alignItems: "center", gap: 6, paddingRight: 8 }, children: [
