...
var rateText = (value) => `${value.toFixed(1)} mm/h`;
var visText = (km) => `${km < 10 ? km.toFixed(1) : String(Math.round(km))} \u516C\u91CC`;
var pctText = (value) => `${Math.round(value)}%`;
...
function roundCoordinates(latitude, longitude) {
  return { latitude: Math.round(latitude * 1e3) / 1e3, longitude: Math.round(longitude * 1e3) / 1e3 };
}
...
async function fetchWeather(location, signal) {
  ...
  const precipIntervalS = typeof current.interval === "number" && Number.isFinite(current.interval) && current.interval > 0 ? current.interval : PRECIP_INTERVAL_FALLBACK_S;
  const precipRate = current.precipitation === null || current.precipitation === void 0 ? void 0 : current.precipitation * 3600 / precipIntervalS;
  const partRate = (value) => value === null || value === void 0 ? void 0 : value * 3600 / precipIntervalS;
  return {
    location,
    current: {
      ...
      visibility: current.visibility === null || current.visibility === void 0 ? void 0 : current.visibility / 1e3,
      dewPoint: current.dew_point_2m ?? void 0,
      precipitation: precipRate,
      rain: partRate(current.rain),
      showers: partRate(current.showers),
      snowfall: current.snowfall ?? void 0,
      cape: current.cape ?? void 0,
      liftedIndex: current.lifted_index ?? void 0,
      freezingLevel: current.freezing_level_height ?? void 0
    },
    ...
    hourly: (hourly?.time ?? []).slice(hourlyFrom, hourlyFrom + 24).map((time, index) => ({
      time,
      temperature: hourly?.temperature_2m?.[hourlyFrom + index] ?? void 0,
      weatherCode: hourly?.weather_code?.[hourlyFrom + index] ?? -1,
      isDay: (hourIsDay[hourlyFrom + index] ?? 1) === 1,
      precipProb: hourly?.precipitation_probability?.[hourlyFrom + index] ?? void 0,
      ...
      precipitation: hourly?.precipitation?.[hourlyFrom + index] ?? void 0,
      showers: hourly?.showers?.[hourlyFrom + index] ?? void 0,
      snowfall: hourly?.snowfall?.[hourlyFrom + index] ?? void 0,
      cape: hourly?.cape?.[hourlyFrom + index] ?? void 0
    })),
    daily: (daily?.time ?? []).slice(0, FORECAST_DAYS).map((date, index) => ({
      date,
      weatherCode: daily?.weather_code?.[index] ?? -1,
      tempMax: daily?.temperature_2m_max?.[index] ?? void 0,
      tempMin: daily?.temperature_2m_min?.[index] ?? void 0,
      precipProb: daily?.precipitation_probability_max?.[index] ?? void 0,
      precipSum: daily?.precipitation_sum?.[index] ?? void 0,
      snowfallSum: daily?.snowfall_sum?.[index] ?? void 0
    })),
...
function rainRateNote(rate) {
  return rate === void 0 ? "" : `\uFF08\u964D\u6C34\u7387 ${rate.toFixed(1)} mm/h\uFF09`;
}
...
function WeatherBar(props) {
  ...
  const visibilityTextValue = cur?.visibility !== void 0 ? `${cur.visibility < 10 ? cur.visibility.toFixed(1) : Math.round(cur.visibility)} km` : void 0;
...
var CLOCK_HOURS = Array.from({ length: 24 }, (_, index) => String(index).padStart(2, "0"));
var CLOCK_MINUTES = Array.from({ length: 12 }, (_, index) => String(index * 5).padStart(2, "0"));
...
function SavedCitiesList(props) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime9.jsxs)("div", { style: { padding: "10px 12px", marginBottom: 8, background: BG_ROW, borderRadius: 10 }, children: [
    ...
    savedCities.saved.length === 0 ? /* @__PURE__ */ (0, import_jsx_runtime9.jsx)("div", { style: { color: MUTED, fontSize: 12 }, children: "\u8FD8\u6CA1\u6709\u6536\u85CF\u57CE\u5E02\u2014\u2014\u641C\u7D22\u57CE\u5E02\u540E\u70B9 \u2606 \u6536\u85CF\uFF0C\u6216\u6536\u85CF\u5F53\u524D\u4F4D\u7F6E\u3002" }) : /* @__PURE__ */ (0, import_jsx_runtime9.jsx)("div", { style: { display: "flex", flexDirection: "column", gap: 6 }, children: savedCities.saved.map((place) => {
